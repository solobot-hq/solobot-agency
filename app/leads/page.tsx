...
// ✅ FIX: Added ': string' type to the parameter to resolve the implicit 'any' error
...
export default function LeadsEnginePage() { 
    ...
    const [leads, setLeads] = useState<any[]>([]); // Added generic type
    ...
    // Add a search to history, maintaining a max of 5
     const addSearchToHistory = (search: any) => {
    ...
    const handleSearch = async (searchParams?: any) => {
        ...
        try {
            ...
            try {
                 ...
                 throw new Error(`Server returned an invalid response (not JSON). Status: ${response?.status || 'unknown'}`);
            }
    ...
    const emailCount = leads.filter(lead => lead.email && lead.email !== 'N/A').length;
                        ...
                        {meta && (
                         ...
                         {banner && (
                             <div className={`mb-4 p-4 rounded-md flex items-start space-x-3 ${
                                 ...
                                 'bg-sky-900/30 border border-sky-500/50 text-sky-300'
                             }`}>
