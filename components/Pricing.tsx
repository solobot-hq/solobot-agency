        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          {Object.entries(PLAN_LIMITS).map(([key, plan]) => (
            <div key={key} className={`relative flex flex-col p-8 rounded-[2rem] border transition-all duration-300 ${key === 'PRO' ? 'bg-[#0D1525] border-indigo-500/40 shadow-2xl shadow-indigo-500/10 scale-[1.01]' : 'bg-[#0D1525] border-white/5 hover:border-white/10'}`}>
              ...
              <Link href="/dashboard/billing" className={`block w-full py-4 rounded-xl text-center font-black text-[10px] uppercase tracking-[0.2em] transition-all active:scale-95 ${key === 'PRO' ? 'bg-indigo-600 text-white hover:bg-indigo-500 shadow-lg shadow-indigo-600/20' : 'bg-white text-black hover:bg-zinc-200'}`}>Start Building Now</Link>
