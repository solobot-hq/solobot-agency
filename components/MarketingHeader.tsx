...
export default function MarketingHeader() {
  ...
  const handleScrollToPricing = (e: React.MouseEvent) => {
    e.preventDefault();
    // Targets the ID "pricing" we will set in the page
    const section = document.getElementById("pricing");
    if (section) {
      section.scrollIntoView({ behavior: "smooth", block: "start" });
    } else {
      // Fallback if the user is on a different page
        ...
        <div className="flex items-center gap-4 md:gap-8">
          ...
          <SignedOut>
            ...
            <button 
              ...
              className="relative z-[110] text-base md:text-xl font-black text-white px-8 py-4 md:px-12 md:py-6 rounded-2xl md:rounded-3xl bg-indigo-600 hover:bg-indigo-700 transition-all shadow-[0_0_40px_rgba(79,70,229,0.3)] whitespace-nowrap cursor-pointer active:scale-95"
            >
              GET STARTED
            </button>
          </SignedOut>
